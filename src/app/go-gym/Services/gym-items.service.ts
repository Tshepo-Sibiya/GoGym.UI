...
export class GymItemsService {
  ...
  getUserGymItems(): Observable<GymItem[]> {
    ...
    return this.http.post<GymItem[]>(this._getUserGymItemsUrl + GYM_API_ENDPOINTS.GET_GYM_ITEMS_BY_USER_ID, user);
  }
  ...
  deleteGymItem(id: any): Observable<any> {
    ...
    return this.http.delete<GymItem[]>(this._getUserGymItemsUrl + GYM_API_ENDPOINTS.DELETE_GYM_ITEM + '?id=' + id);
  }
  ...
  createGymItem(gymItem: GymItem): Observable<any>{
    ...
    //   'Authorization': `Bearer ${token.token}`
    // });
    return this.http.post<GymItem[]>(this._getUserGymItemsUrl + GYM_API_ENDPOINTS.CREATE_GYM_ITEM, gymItem);
  }
}
